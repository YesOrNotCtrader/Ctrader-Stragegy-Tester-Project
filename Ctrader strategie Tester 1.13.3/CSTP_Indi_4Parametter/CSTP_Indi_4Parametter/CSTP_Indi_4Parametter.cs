        ...
        public override void Calculate(int index)
        {
            switch (IndicatorSelection)
            {
                ...
                case (EnumIndiSelection.ADX):
                    Result[index] = Indicators.MovingAverage(Indicators.AverageDirectionalMovementIndexRating((int)Value1).DIPlus, (int)Value2, MaType).Result[index];
                    Signal[index] = Indicators.MovingAverage(Indicators.AverageDirectionalMovementIndexRating((int)Value1).DIMinus, (int)Value2, MaType).Result[index];
                    break;
